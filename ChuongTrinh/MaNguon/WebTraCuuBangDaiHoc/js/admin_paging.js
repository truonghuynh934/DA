...
function totalPages() {
    ...
    xmlhttp.open("GET", "./be/total_pages.php", false);
    xmlhttp.send();
...
function change(indexPage) {
    ...
    // vì hàm này chạy ngay khi load trang nên các input chưa được gán value, dẫn đến null
    // khi chọn search thì các input này mới được gán giá trị (user.html)
    if (idCertificate==null) idCertificate = "";
    if (name == null) name ="";
    ...
    if (idCertificate == "" && name == "" && dateOfBirth == "") {
        ...
        xmlhttp.open("GET", "./be/create_table_paging.php?page=" + indexPage, true);
        xmlhttp.send();
        // console.info("RỖNG");
    } else {
        ...
        xmlhttpSearch.open("GET","./be/search_student_admin.php?page="+indexPage+"&&id_certificate="+idCertificate+"&&name="+name+"&&date_of_birth="+dateOfBirth,true);
        xmlhttpSearch.send();
